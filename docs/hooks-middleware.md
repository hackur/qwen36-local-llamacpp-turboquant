...
\* `tier1+hooks` falls back to `tier1-only` until #39 lands (the hook engine is being implemented in `proxy/src/hooks/` by a sibling task). Numbers in those rows are placeholders for the hook-engine variant and should match `tier1-only` exactly until the hook dispatcher activates.
