...
RESP=$(curl -sf "$PROXY/v1/chat/completions" \
  ...
  -d '{"model":"qwen3.8-local","messages":[{"role":"user","content":"Reply with the single word: pong"}],"max_tokens":16,"chat_template_kwargs":{"enable_thinking":false}}')
...
STREAM=$(curl -sN "$PROXY/v1/chat/completions" \
  -H "content-type: application/json" \
  -d '{"model":"qwen3.8-local","messages":[{"role":"user","content":"Count to three."}],"max_tokens":32,"stream":true,"chat_template_kwargs":{"enable_thinking":false}}')
...
TOOL_REQ='{"model":"qwen3.8-local","messages":[{"role":"user","content":"Use the get_weather tool to get the weather in Paris."}],"tools":[{"type":"function","function":{"name":"get_weather","description":"Get weather for a city","parameters":{"type":"object","properties":{"city":{"type":"string"}},"required":["city"]}}}],"tool_choice":"auto","max_tokens":150,"chat_template_kwargs":{"enable_thinking":false}}'
...
ROUND='{"model":"qwen3.8-local","messages":[{"role":"user","content":"What is the weather in Paris?"},{"role":"assistant","content":"","tool_calls":[{"id":"call_1","type":"function","function":{"name":"get_weather","arguments":"{\"city\":\"Paris\"}"}}]},{"role":"tool","tool_call_id":"call_1","name":"get_weather","content":"{\"temp_c\":18,\"conditions\":\"partly cloudy\"}"}],"tools":[{"type":"function","function":{"name":"get_weather","description":"Get weather for a city","parameters":{"type":"object","properties":{"city":{"type":"string"}},"required":["city"]}}}],"max_tokens":80,"chat_template_kwargs":{"enable_thinking":false}}'
...
curl -sf -o /tmp/qwen-compact-debug-body.json -D /tmp/qwen-compact-headers.txt \
  ...
  -d '{"model":"qwen3.8-local","messages":[{"role":"user","content":"hi"}],"max_tokens":4,"chat_template_kwargs":{"enable_thinking":false}}' \
