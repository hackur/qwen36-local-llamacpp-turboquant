...
| `caveman-self-compact` | `decision-heavy.jsonl` | 100 | 100 | +0.0 | 70 | 70 | 0.0 | no-op (both) |
| `caveman-self-compact` | `mixed-prose-tool.jsonl` | 100 | 100 | +0.0 | 72 | 72 | 0.0 | loses to baseline |
| `caveman-self-compact` | `tool-heavy.jsonl` | 100 | 100 | +0.0 | 71 | 71 | 0.0 | loses to baseline |
| `do-nothing` | `chit-chat.jsonl` | 100 | 100 | +0.0 | 73 | 73 | 0.0 | no-op (both) |
| `do-nothing` | `code-review.jsonl` | 100 | 100 | +0.0 | 70 | 70 | 0.0 | no-op (both) |
| `do-nothing` | `decision-heavy.jsonl` | 100 | 100 | +0.0 | 69 | 69 | 0.0 | no-op (both) |
| `do-nothing` | `mixed-prose-tool.jsonl` | 100 | 100 | +0.0 | 71 | 71 | 0.0 | loses to baseline |
| `do-nothing` | `tool-heavy.jsonl` | 100 | 100 | +0.0 | 70 | 70 | 0.0 | loses to baseline |
| `tier0+tier1` | `chit-chat.jsonl` | 100 | 100 | +0.0 | 72 | 72 | 0.0 | baseline |
| `tier0+tier1` | `code-review.jsonl` | 100 | 100 | +0.0 | 73 | 73 | 0.0 | baseline |
| `tier0+tier1` | `decision-heavy.jsonl` | 100 | 100 | +0.0 | 70 | 70 | 0.0 | baseline |
...
| `tier0-only` † | `chit-chat.jsonl` | 100 | 100 | +0.0 | 72 | 72 | 0.0 | no-op (both) |
| `tier0-only` † | `code-review.jsonl` | 100 | 100 | +0.0 | 73 | 73 | 0.0 | no-op (both) |
| `tier0-only` † | `decision-heavy.jsonl` | 100 | 100 | +0.0 | 68 | 68 | 0.0 | no-op (both) |
| `tier0-only` † | `mixed-prose-tool.jsonl` | 100 | 100 | +0.0 | 69 | 69 | 0.0 | loses to baseline |
| `tier0-only` † | `tool-heavy.jsonl` | 100 | 100 | +0.0 | 70 | 70 | 0.0 | loses to baseline |
| `tier1-only` † | `chit-chat.jsonl` | 100 | 100 | +0.0 | 70 | 70 | 0.0 | no-op (both) |
| `tier1-only` † | `code-review.jsonl` | 100 | 100 | +0.0 | 71 | 71 | 0.0 | no-op (both) |
| `tier1-only` † | `decision-heavy.jsonl` | 100 | 100 | +0.0 | 1 | 1 | 0.0 | no-op (both) |
| `tier1-only` † | `mixed-prose-tool.jsonl` | 100 | 100 | +0.0 | 74 | 74 | 0.0 | loses to baseline |
| `tier1-only` † | `tool-heavy.jsonl` | 100 | 100 | +0.0 | 71 | 71 | 0.0 | loses to baseline |
...
| `tier1+hooks` \* | `mixed-prose-tool.jsonl` | 100 | 100 | +0.0 | 71 | 71 | 0.0 | loses to baseline |
| `tier1+hooks` \* | `tool-heavy.jsonl` | 100 | 100 | +0.0 | 69 | 69 | 0.0 | loses to baseline |
...
\* `tier1+hooks` falls back to `tier1-only` because the A/B harness shim doesn't yet route through the hook engine. The engine itself has landed (`proxy/src/hooks/engine.js`); what's missing is wiring `proxy/src/hooks/` into `proxy/scripts/run-rewrite.js` and re-running the sweep. Numbers in those rows are placeholders for the hook-engine variant and should match `tier1-only` exactly until that re-run happens.

† `tier0-only` and `tier1-only` show identical, no-op results in this run because the current `proxy/src/rewrite.js` does not expose a single-tier mode: Tier 0 (the verbatim window in `verbatim.js`) and Tier 1 (the stub-replace pass in `tier1.js`) are sequential — Tier 0 selects evictable candidates that Tier 1 then stubs. The harness shim approximates "tier-N off" by widening the relevant watermark; with one tier off there are no candidates for the other tier to act on. Once the hook engine lands these single-tier legs become independently meaningful (the hooks system can run Tier 1 against an unrestricted candidate set, see compaction-strategy.md §7).

The smoke run uses the deterministic chars/4 stub tokenizer; latencies above are pure-CPU rewrite-pipeline numbers (no upstream model call), so the p50/p99 columns reflect rewrite cost only — not end-to-end request latency. The hook-time columns from §11's metric block (`total_hook_time_p50_ms`, `total_hook_time_p99_ms`) are absent here because the dispatcher is not yet wired; the aggregator emits 0 for those fields. See `proxy/eval/ab-harness/README.md` for re-running.
