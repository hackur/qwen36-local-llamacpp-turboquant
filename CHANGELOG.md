...
- **Qwen3.8-27B Q8_0 is the canonical local model.** `qwen38-27b` now drives `make start`, editor configs, the offline launcher, native 262K context, vision, and TurboQuant embedded-MTP decoding. Qwen3.6 models remain explicit fallbacks.
- **Reproducible current-engine upgrades.** `configs/upstream.env` records the accepted mainline and TurboQuant revisions; `make upgrade` updates both clean vendor checkouts to their configured upstream branches and rebuilds them together.
- **Current runtime dependencies.** The proxy now uses `js-yaml` 5.3.0 and `pino` 10.3.1; the Python TUI/proxy ranges were refreshed to compatible current releases.
...
- **`scripts/start-fallback.sh` was missing `chat_template_flags`.** Every other Qwen-aware start script applies the froggeric v19 chat template via the `chat_template_flags` helper; the fallback path only passed `--jinja`, so the model's embedded template was used and the empty-think / KV-cache-invalidation / tool-call XML parsing bugs the v19 template fixes were silently re-introduced when a user fell back to the q8_0 KV path with a Qwen alias.
- **`scripts/bench-ab.sh median()` silently returned `0.00` when every run failed.** Now emits `ERROR` from awk on empty input and the per-side / summary callers propagate it instead of reporting fake zero tok/s.
- **`scripts/bench_suite.py` validator missed runtime-deadlocking suites.** Added: (a) reject any job with `a.port == b.port` (would otherwise deadlock at `waiting_port` forever under the one-side-only-listens rule); (b) resolve and check per-job / defaults `prompt_file` at validate time so a missing prompt file fails fast at load instead of mid-run.
- **`scripts/start-vision.sh`, `scripts/start-embed.sh` lacked `--dry-run`.** Now parse `--dry-run` like the other four start scripts, wrap `ensure_no_other_llama_server` / `ensure_port_free` / `preflight_memory` behind it, and emit the final argv via `printf %q` without launching.

### Changed

- **Both inference engines refreshed to their 2026-08-20 tips.** TurboQuant `bd1bf025` fixes Qwen3.8 loading and adds embedded MTP support; llama.cpp mainline is `681c29d3`. Localhost builds explicitly disable OpenSSL because this project does not expose TLS.
- **`scripts/mcp-bridge.sh` auto-detects `MCP_CORS`.** Previously hardcoded to `http://127.0.0.1:10501`, so baseline (`:10500`) users had to set the env var manually. Now scans `:10500/10501/10502/10503` for a listening llama-server and picks the lowest, with `:10501` as the fallback. `MCP_CORS` override and the `--help` text both document the new behavior.

### Docs

- **`README.md`** — added "Since v0.0.2" section (TUI, MCP, single-server guard), two new "What can you do with this?" rows (interactive bench, MCP tools), retitled the trailer to "Highlights from v0.0.2", replaced the stale "All targets" list with `Run \`make help\`` (auto-fresh).
- **`docs/mcp-integration.md`** — softened the misleading "Investigation complete" opener to "Investigation complete; integration in progress" + an explicit Shipped / Pending status block.
- **`CONTRIBUTING.md`** — new "Contributing a bench suite" subsection covering `benchmarks/suites/*.yaml`, the validator command, and the protocol cross-reference.
- **`docs/benchmarking-discipline.md`** — migrated the P5 runner-protocol spec (events.jsonl / control.jsonl schemas, phase state machine, run-dir layout, lockfile rule) out of TODO.md so the roadmap can eventually be pruned.
